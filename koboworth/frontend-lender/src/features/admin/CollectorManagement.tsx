...
export const CollectorManagement: React.FC = () => {
  ...
  useEffect(() => {
    api.get('/admin/collectors').then(res => setCollectors(res.data)).catch(console.error);
  }, []);

  const toggleSuspension = async (id: string | number) => {
    try {
      await api.post(`/admin/collectors/${id}/toggle-status`);
