    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          <div className="bg-blue-100 p-4 rounded-lg"><Activity className="text-blue-600" /></div>
          <div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          <div className="bg-green-100 p-4 rounded-lg"><Database className="text-green-600" /></div>
          <div>
            <p className="text-gray-500 text-sm">Remaining Quota</p>
            <p className="text-2xl font-bold">{stats.quotaRemaining}</p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          <div className="bg-orange-100 p-4 rounded-lg"><AlertTriangle className="text-orange-600" /></div>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 lg:col-span-1">
          <h2 className="font-semibold mb-4 text-gray-700">Tier Distribution</h2>
          <div className="h-64">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 lg:col-span-2">
          <h2 className="font-semibold mb-4 text-gray-700">Query History</h2>
              ...
              <tbody>
                {stats.history.map((row: any) => (
                  <tr key={row.id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-gray-500 text-sm">{row.time}</td>
                    <td className="py-3 px-4">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${row.status === 'SUCCESS' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {row.status}
                      </span>
                    </td>
                  </tr>
